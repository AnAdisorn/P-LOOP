...
lamb=int(Arg[0])                               #Sets the wavelength pre-factor
...
np.save('exctn1'+str(lamb), data)
np.save('intracurr1'+str(lamb), data1)
np.save('intercurr1'+str(lamb), data2)
np.save('totcurr1'+str(lamb), data3)

Dataout1=np.column_stack((Q4,H1))  #intraband
Dataout2=np.column_stack((Q4,H2))  #interband
Dataout3=np.column_stack((Q4,H3))  #total
Dataout4=np.column_stack((Q4,Q))   #total excitation of conduction band

np.savetxt('acurrent1'+str(lamb)+'.txt' ,Dataout1)
np.savetxt('bcurrent1'+str(lamb)+'.txt' ,Dataout2)
np.savetxt('ccurrent1'+str(lamb)+'.txt' ,Dataout3)
np.savetxt('sumextn1'+str(lamb)+'.txt',Dataout4)
